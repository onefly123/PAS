...
# PAS needed
@click.option('--plug_solver', 'plug_solver',help='Plug solver', metavar='euler|ipndm...',                           type=click.Choice(['euler', 'deis']), required=True)
...
def main(dataset_name, plug_solver, epochs, early_stop, learning_rate, PCA_num, first_time_tau, others_time_tau, para_name, use_xT, loss_type, tea_traj_dir, device=torch.device('cuda'), **solver_kwargs):
    ...
    elif plug_solver == 'deis':
        plug_solver_fn = plug_solvers.deis_step
    else:
    ...
    if plug_solver == 'deis':
        ...
        coeff_list = solver_utils.get_deis_coeff_list(stu_t_step, solver_kwargs['max_order'], deis_mode=solver_kwargs["deis_mode"])
    ...
    buffer_deis = [[d_first] for d_first in d_first_list]  # Initialize the buffer_ipndm, used to save the d of the previous time point (only used for ipndm), batch, [batch_size, c, r, r]
        ...
        if flag_need_search == 1:  # If the current time point needs to be corrected
                ...
                if solver_kwargs['model_source'] == 'ldm':
                    with autocast("cuda"):
                        with net.model.ema_scope():
                            for cur_xt_searched_batch, class_labels_batch, cond_batch, uncond_cond_batch, buffer_deis_batch in zip(cur_xt_searched, class_labels_list, cond_list, uncond_cond_list, buffer_deis):
                                if plug_solver == "euler":
                                    ...
                                    cur_d_batch = plug_solver_fn(coeff_list, device_, cur_xt_searched_batch, stu_t_step[time_point+1], stu_t_step[time_point+2], solver_kwargs['model_source'], net, time_point, buffer_deis_batch, max_order, class_labels_batch, cond_batch, uncond_cond_batch)
                                else:
                                    raise ValueError(f'Unknown method: {plug_solver}')
                                cur_d_temp.append(cur_d_batch)
                else:
                    for cur_xt_searched_batch, class_labels_batch, buffer_deis_batch in zip(cur_xt_searched, class_labels_list, buffer_deis):
                        if plug_solver == "euler":
                            ...
                            cur_d_batch = plug_solver_fn(coeff_list, device_, cur_xt_searched_batch, stu_t_step[time_point+1], stu_t_step[time_point+2], solver_kwargs['model_source'], net, time_point, buffer_deis_batch, max_order, class_labels_batch)
                        else:
                            raise ValueError(f'Unknown method: {plug_solver}')
                        cur_d_temp.append(cur_d_batch)
            cur_d = cur_d_temp
            ...
            with torch.no_grad():
                ...
                if solver_kwargs['model_source'] == 'ldm':
                    with autocast("cuda"):
                        with net.model.ema_scope():
                            for cur_xt_searched_batch, cur_d_batch, class_labels_batch, cond_batch, uncond_cond_batch, buffer_deis_batch in zip(cur_xt_searched, cur_d, class_labels_list, cond_list, uncond_cond_list, buffer_deis):
                                # Euler step.
                                ...
                                if plug_solver == "euler":
                                    ...
                                    cur_d_batch = plug_solver_fn(coeff_list, device_, next_xt_batch, stu_t_step[time_point+1], stu_t_step[time_point+2], solver_kwargs['model_source'], net, time_point, buffer_deis_batch, max_order, class_labels_batch, cond_batch, uncond_cond_batch)
                                else:
                                    raise ValueError(f'Unknown method: {plug_solver}')
                                cur_d_temp.append(cur_d_batch)
                else:
                    for cur_xt_searched_batch, cur_d_batch, class_labels_batch, buffer_deis_batch in zip(cur_xt_searched, cur_d, class_labels_list, buffer_deis):
                        # Euler step.
                        next_xt_batch = cur_xt_searched_batch + cur_d_batch * (next_stu_t_step - cur_stu_t_step)
                        cur_xt_searched_temp.append(next_xt_batch)
                        if plug_solver == "euler":
                            ...
                            cur_d_batch = plug_solver_fn(coeff_list, device_, next_xt_batch, stu_t_step[time_point+1], stu_t_step[time_point+2], solver_kwargs['model_source'], net, time_point, buffer_deis_batch, max_order, class_labels_batch)
                        else:
                            raise ValueError(f'Unknown method: {plug_solver}')
                        cur_d_temp.append(cur_d_batch)
